...
_ = Task.Run(async () =>
{
    try
    {
        ...
        if (dbProvider.Equals("pgsql", StringComparison.OrdinalIgnoreCase))
        {
            // Check if we should skip column fixes (can be set if migrations are causing issues)
            var skipColumnFixes = Environment.GetEnvironmentVariable("SKIP_COLUMN_FIXES")?.ToLower() == "true";

            if (skipColumnFixes)
            {
                Console.WriteLine("Skipping PostgreSQL column fixes (SKIP_COLUMN_FIXES=true)");
            }
            else
            {
                try
                {
                    // Set a longer timeout for these operations BEFORE any commands
                    context.Database.SetCommandTimeout(180); // 3 minutes

                    // Fix auto-increment for all primary key columns
                    var identityCommands = new[]
                {
                ...
                Console.WriteLine("Applying identity column fixes...");
                foreach (var sql in identityCommands)
                {
                    try
                    {
                        context.Database.ExecuteSqlRaw(sql);
                        Console.WriteLine($"✓ {sql.Substring(12, 30)}...");
                    }
                    catch (Exception ex)
                    {
                        // Column might already be configured - this is fine
                        Console.WriteLine($"- Skipped: {sql.Substring(12, 30)}... (already configured)");
                    }
                }

                // Timestamp conversions - these can be slow, so we skip them if they're already correct
                Console.WriteLine("Checking timestamp columns (skipping if already timestamptz)...");
                var timestampCommands = new[]
                {
                ...
                // Execute timestamp conversions with individual timeout handling
                var successCount = 0;
                var skipCount = 0;
                foreach (var sql in timestampCommands)
                {
                    try
                    {
                        // Each command gets 180 seconds
                        context.Database.ExecuteSqlRaw(sql);
                        successCount++;
                        Console.WriteLine($"✓ Converted: {sql.Substring(12, 40)}...");
                    }
                    catch (Exception ex)
                    {
                        skipCount++;
                        // This is expected if column is already timestamptz or doesn't exist
                        if (ex.Message.Contains("timeout") || ex.Message.Contains("Exception while reading"))
                        {
                            Console.WriteLine($"⚠ Timeout (skipping): {sql.Substring(12, 40)}... - consider manual migration");
                        }
                        else
                        {
                            Console.WriteLine($"- Skipped: {sql.Substring(12, 40)}... (already configured)");
                        }
                    }
                }

                    Console.WriteLine($"Column fixes complete: {successCount} conversions, {skipCount} skipped");
                }
                catch (Exception ex)
                {
                    Console.WriteLine($"Error during PostgreSQL column fixes: {ex.Message}");
                    Console.WriteLine("Application will continue - column fixes are optional");
                }
                finally
                {
